...
- **Rationale (risk-minimizing):** Amazon and BestBuy are the two LIVE-TESTED plugins with proven timing behavior. Choosing their existing field names as canonical leaves those two plugins UNCHANGED (zero risk to proven behavior) and limits code changes to the 5 non-live-verified community plugins (Walmart, Target, GameStop, SquareEnix, NewEgg), which currently use `min_delay` / `max_delay`. This overrides the "majority uses min_delay/max_delay (5/7)" and the source comment labeling delay_seconds/delay_jitter "legacy" — protecting proven live behavior is the stronger criterion.
