...
- [x] **RH-02**: release-please workflow + config tags releases and maintains `CHANGELOG.md` from conventional commits, seeded at product version **v2.0.0** (SEED-002)
- [x] **RH-03**: `pyproject.toml` canonical version is reconciled to `2.0.0` so release-please (python type) has a correct source of truth
...
- [x] **AF-02**: `get_status()` and SSE status frames no longer expose the raw `last_heartbeat` monotonic float (only derived `heartbeat_age_secs`)
...
- [x] **FC-02**: Operator can view outcome analytics (success-rate, time-to-checkout) computed over existing BUY-04 verified-order records
...
| RH-04 | Phase 31 | Complete |
| RH-05 | Phase 31 | Complete |
