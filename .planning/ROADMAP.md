...
- [ ] 38-03-PLAN.md: workflow hardening. `permissions` block in `ci.yml`, gated removal of `codeql-analysis.yml`, every `uses:` pinned to a verified SHA with a trailing tag comment, SCAN-08's deferral recorded in four files (`.planning/REQUIREMENTS.md`, this file's Phase 38 success criterion 2, `.planning/STATE.md`, `38-SCAN-BASELINE.md`) (SCAN-05, SCAN-06, SCAN-08, SCAN-11)
- [ ] 38-04-PLAN.md: land the branch by pull request, then wait for the post-merge CodeQL analysis and re-query alerts 3, 4, 5 by number, proving SCAN-05 by the observable `ci.yml` permissions condition rather than by alert number. No Dependabot re-query: `#13` closed on 2026-09-07 and the open count is 0 (SCAN-01, SCAN-03, SCAN-05, SCAN-06, SCAN-11)
...
**Planning corrections** (verified live 2026-08-03 against the GitHub API, these supersede the success criteria above where they conflict):
  ...
  - **Superseding update, verified live 2026-09-07:** Dependabot `#13` is now CLOSED. It reads `state: fixed`, `fixed_at: 2026-09-07T04:02:21Z`, closed by merging PR #26 (`chore(deps): bump cryptography from 49.0.0 to 50.0.0`) onto `master` at `ecbdfcc`, and `master`'s `requirements.txt` now pins `cryptography==50.0.0`. The open Dependabot count is **0**, so SCAN-01 is already satisfied and 38-02's hand-written cryptography bump is redundant.
