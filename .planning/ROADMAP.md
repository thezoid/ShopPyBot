...
- [x] 07-03-PLAN.md — main.py thin shim routes through BotService.run; update test_main_wiring delegation seam (MOD-02, MOD-03)
...
- [x] 08-02-PLAN.md — KeyringBackend + EncryptedFileBackend (Fernet/scrypt) + keyring pin (CRED-02, CRED-03)
- [x] 08-03-PLAN.md — Backend selection precedence + backend-name logging + BotService init_store + no-plaintext test (CRED-05, CRED-06)
- [x] 08-04-PLAN.md — Migrate 16 os.environ secret reads to get_store + migrate_from_env + --migrate + SC1 grep guard (CRED-01, CRED-07)
...
- [ ] 09-01-PLAN.md — CLI package scaffold: argparse subparsers + dispatch + run handler + Wave-0 test scaffold (CLI-01)
- [ ] 09-02-PLAN.md — setup (getpass credential store + backend write + --migrate alias) + config show/set allowlist (CLI-02, CLI-03)
- [ ] 09-03-PLAN.md — items list/add/remove over BotService + MOD-02 AST grep guard (CLI-03, MOD-02)
