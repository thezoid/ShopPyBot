"""Tests for plugins/shopbot_plugin_bestbuy.py -- _last_tab + get_active_tab() (BUY-03).

Covers:
- BestBuyPlugin.get_active_tab() returns self._last_tab when set
- BestBuyPlugin.get_active_tab() falls back to driver.main_tab when _last_tab not set
- BestBuyPlugin.get_active_tab() returns None when both _last_tab and main_tab absent
...
def _make_bestbuy_plugin():
    """Build a BestBuyPlugin instance without launching a real browser."""
    from plugins.shopbot_plugin_bestbuy import BestBuyPlugin

    instance = BestBuyPlugin(config=None)
