...
class GameStopPlugin(RetailerPlugin):
    ...
    async def setup(self) -> None:
        ...
        if getattr(self, "_proxy_required", False) and getattr(self, "_proxy", None) is None:
            ...
            raise RuntimeError("GameStopPlugin: proxy pool exhausted; cannot launch")
    ...
    async def login(self) -> bool:
        ...
        store = get_store()
        email = store.get("GAMESTOP_EMAIL") or ""
        password = store.get("GAMESTOP_PASSWORD") or ""
        ...
        try:
            ...
            verified = await self._verify_login_generic(tab, "/login", "input#login-form-email")
            if not verified:
                writeLog("GameStop login verification failed", "WARNING")
                return False
            return True
        except Exception as exc:
    ...
    async def auto_buy(self, url: str) -> bool:
        ...
        try:
            ...
            if not login_ok:
                writeLog("GameStop login failed during auto_buy -- aborting checkout", "ERROR")
                return False
